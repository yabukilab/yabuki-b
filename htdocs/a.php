...
try {
    ...
    if (isset($_POST['circle']) && $_POST['circle'] == '0') {
        ...
        if ($row_count['count'] < 2) {
            ...
            // 赤い丸の座標をデータベースに挿入
            $sql_insert_circle = "INSERT INTO red_circles (x_position, y_position) VALUES (?, ?)";
            $stmt = $conn->prepare($sql_insert_circle);
            $stmt->bindParam(1, $x_position, PDO::PARAM_INT);
            $stmt->bindParam(2, $y_position, PDO::PARAM_INT);

            if ($stmt->execute()) {
                echo "";
            } else {
                echo "エラー: " . $sql_insert_circle . "<br>" . $stmt->errorInfo()[2];
            }
        }
    }
    ...
    if (isset($_POST['reset_circles'])) {
        ...
        if ($conn->exec($sql_reset) !== false) {
            echo "OUTカウントがリセットされました";
        } else {
            echo "エラー: " . $sql_reset . "<br>" . $conn->errorInfo()[2];
        }
    }
    ...
    if (isset($_POST['reset_scores'])) {
        ...
        if ($conn->exec($sql_reset_scores) !== false) {
            echo "得点データがリセットされました";
        } else {
            echo "エラー: " . $sql_reset_scores . "<br>" . $conn->errorInfo()[2];
        }
    }
    ...
    if (isset($_POST['image'])) {
        ...
        if ($stmt->execute()) {
            ...
            echo "画像の保存に失敗しました: " . $stmt->errorInfo()[2];
        }
    }
...
<div class="red-circles">
    ...
    if ($result_circles->rowCount() > 0) {
        while ($row = $result_circles->fetch(PDO::FETCH_ASSOC)) {
